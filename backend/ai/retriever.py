...
def resolve_target_node(graph, question: str, context_node_id: str | None = None) -> dict | None:
    ...
    Falls back to `context_node_id` (the node currently selected/discussed
    in the UI) when the question names nothing the graph recognizes —
    without this, referential follow-ups like "what's the risk here" or
    "why is this a problem" have no way to resolve, since there's no name
    in the question at all for find_nodes_by_name to match against.
    """
    matches = find_nodes_by_name(graph, question, limit=1)
    if matches:
        return matches[0]
    if context_node_id:
        return get_node(graph, context_node_id)
    return None


def retrieve_for_impact(graph, question: str, repo_root: str | None = None, context_node_id: str | None = None) -> dict:
    target = resolve_target_node(graph, question, context_node_id)
    if target is None:
...
def retrieve_for_bug_trace(graph, question: str, repo_root: str | None = None, context_node_id: str | None = None) -> dict:
    """Entry point is resolved the same way as an impact target: cheap name
    match against API_ROUTE/FUNCTION nodes, falling back to the UI's current
    context node. Traces forward over CALLS edges.
    """
    entry = resolve_target_node(graph, question, context_node_id)
    if entry is None:
...
def retrieve_for_general_qa(
    graph, question: str, repo_root: str | None = None, hops: int = 1, context_node_id: str | None = None
) -> dict:
    seeds = find_nodes_by_name(graph, question, limit=3)
    seed_ids = [n["id"] for n in seeds]
    if not seed_ids and context_node_id and get_node(graph, context_node_id):
        # No name in the question matched anything — fall back to whatever
        # node the UI says is currently in view, so "explain this" or
        # "what's risky about it" resolve instead of dead-ending.
        seed_ids = [context_node_id]
    if not seed_ids:
