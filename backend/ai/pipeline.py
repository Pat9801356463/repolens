...
def answer_question(question: str, graph, llm_client: LLMClient, repo_root: str | None = None) -> dict:
    ...
    if intent == "impact_analysis":
        return _handle_impact(question, graph, llm_client, repo_root)
    if intent == "bug_trace":
        return _handle_bug_trace(question, graph, llm_client, repo_root)
    if intent == "test_gen":
        return _safe_result(
    ...
    return _handle_general_qa(question, graph, llm_client, repo_root)
...
def _handle_impact(question: str, graph, llm_client: LLMClient, repo_root: str | None) -> dict:
    data = retriever.retrieve_for_impact(graph, question, repo_root=repo_root)
    if data["target"] is None:
        return _safe_result(
            "I couldn't find a matching function, class, or file in the graph "
            "for that question — try naming it more specifically."
        )
...
def _handle_bug_trace(question: str, graph, llm_client: LLMClient, repo_root: str | None) -> dict:
    data = retriever.retrieve_for_bug_trace(graph, question, repo_root=repo_root)
    if data["entry"] is None:
        return _safe_result(
            "I couldn't find a matching entry point in the graph for that "
            "question — try naming the route or function involved."
        )
...
def _handle_general_qa(question: str, graph, llm_client: LLMClient, repo_root: str | None) -> dict:
    data = retriever.retrieve_for_general_qa(graph, question, repo_root=repo_root)
    if not data["node_ids"]:
        return _safe_result(
            "I couldn't find anything in the codebase graph matching that "
            "question — try naming a specific file, class, or function."
        )
